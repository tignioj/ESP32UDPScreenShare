#ifndef MY_SCALE_FUNCTION_H
#define MY_SCALE_FUNCTION_H
...
// ================= 放大映射表 =================
static int scale_x_map[240];
static int scale_y_map[240];

static inline void init_scale_maps()
{
    for (int i = 0; i < 240; i++) {
        scale_x_map[i] = (i * 180 + 120) / 240;
        if (scale_x_map[i] >= 180) scale_x_map[i] = 179;

        scale_y_map[i] = (i * 180 + 120) / 240;
        if (scale_y_map[i] >= 180) scale_y_map[i] = 179;
    }
}

// ================= 180 → 240（RGB565 only） =================
static inline void scale_180_to_240_table(
    const uint16_t* src,
    uint16_t* dst,
    int src_lines
)
{
    ...
    for (int dy = 0; dy < dst_lines; dy++) {
        int sy = scale_y_map[dy];
        if (sy >= src_lines) sy = src_lines - 1;

        const uint16_t* src_line = src + sy * 180;
        uint16_t* dst_line = dst + dy * 240;

        for (int dx = 0; dx < 240; dx++) {
            dst_line[dx] = src_line[scale_x_map[dx]];
        }
    }
}

// ================= 120 → 240（RGB565 only） =================
static inline void scale_120_to_240(
    const uint16_t* src,
    uint16_t* dst,
    int src_lines
)
{
    for (int sy = 0; sy < src_lines; sy++) {
        const uint16_t* src_line = src + sy * 120;

        uint16_t* dst0 = dst + (sy * 2)     * 240;
        uint16_t* dst1 = dst + (sy * 2 + 1) * 240;

        for (int sx = 0; sx < 120; sx++) {
            uint16_t p = src_line[sx];
            int dx = sx * 2;
